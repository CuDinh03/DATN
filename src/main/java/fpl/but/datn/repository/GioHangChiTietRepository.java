...
public interface GioHangChiTietRepository extends JpaRepository<GioHangChiTiet, UUID> {
    ...
    @Query(value = "SELECT ghct.* FROM gio_hang_chi_tiet ghct INNER JOIN gio_hang gh ON ghct.gio_hang_id = gh.id WHERE gh.id = :idGioHang ORDER BY ghct.ngay_tao DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT ghct.*, ha.* FROM gio_hang_chi_tiet ghct " +
            ...
            "WHERE gh.id = :idGioHang ORDER BY ghct.ngay_tao DESC", nativeQuery = true)
