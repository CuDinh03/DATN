...
public interface GioHangHoaDonRepository extends JpaRepository<GioHangHoaDon, UUID> {
    @Query(value = "SELECT hdgh.* FROM gio_hang_hoa_don hdgh INNER JOIN hoa_don hd ON hdgh.id_hoa_don = hd.id WHERE hd.id = :idHoaDon AND hd.trang_thai = 1", nativeQuery = true)
